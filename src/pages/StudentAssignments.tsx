...
// Relevance Config for evaluation
const RELEVANCE_CONFIG = {
  agent: {
    endpoint: "https://api-d7b62b.stack.tryrelevance.com/latest/agents/trigger",
    authorization: "5cc7752400a6-4648-b47b-04fc92b47cae:sk-OGIxMWJiMzAtMTk5Ni00Nzk3LTk5MTYtZTFmZTI4NzIzNTNj",
  ...
  tools: {
    ...
    detectPlagiarism: {
      endpoint:
        "https://api-d7b62b.stack.tryrelevance.com/latest/studios/8679da8e-337d-4bfe-b185-7844e00eaee6/trigger_webhook?project=5cc7752400a6-4648-b47b-04fc92b47cae",
      authorization: "5cc7752400a6-4648-b47b-04fc92b47cae:sk-MjM0MWUwZmEtZDk0Zi00Y2Q5LWE2ZGYtNjA5NTFjM2VlMmU1",
    },
    detectAIContent: {
      endpoint:
        "https://api-d7b62b.stack.tryrelevance.com/latest/studios/ebb200c3-83be-473e-88e3-b83d24c06346/trigger_webhook?project=5cc7752400a6-4648-b47b-04fc92b47cae",
      authorization: "5cc7752400a6-4648-b47b-04fc92b47cae:sk-NmMwNmE3OGYtNzg5My00MDJjLTkxODYtYzA2NTE2NjViZWNi",
    },
  },
  region: "d7b62b",
...
export default function StudentAssignmentsPage() {
  ...
  const uploadFileToSupabase = async (file: File): Promise<string> => {
    try {
      ...
      const supabaseUrl = "https://vnyoexxeqdjpzekjteft.supabase.co"
      const supabaseAnonKey =
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZueW9leHhlcWRqcHpla2p0ZWZ0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDg5MzQxMTUsImV4cCI6MjA2NDUxMDExNX0.Ta1LilxCNkG_9SX3NEweCU-VrO7xTTXrKMJvsmarghA"
      const supabase = createClient(supabaseUrl, supabaseAnonKey)

      const timestamp = Date.now()
      const fileName = `assignments/${timestamp}-${file.name.replace(/[^a-zA-Z0-9.-]/g, "_")}`

      const { data, error } = await supabase.storage.from("assignment-files").upload(fileName, file, {
        cacheControl: "3600",
        upsert: false,
      })
      ...
      const { data: urlData } = supabase.storage.from("assignment-files").getPublicUrl(data.path)
  ...
  const handleSubmitAssignment = async (): Promise<void> => {
    ...
    try {
      ...
      // Send to AI for background analysis (don't wait for detailed results)
      const analysisMessage = `Please analyze this assignment submission for "${selectedAssignment.title}":
...
Document URL: ${fileUrl}`
      ...
      // Start AI analysis in background (fire and forget)
      callRelevanceAgent(analysisMessage).catch(console.error)

      // Update assignment status immediately
      setAssignments(
        assignments.map((assignment) =>
          assignment.id === selectedAssignment.id
            ? { ...assignment, status: "submitted", submittedAt: new Date().toISOString() }
      ...
      toast({
        title: "Assignment submitted successfully! 🎉",
        description: "Your assignment has been submitted and is being processed by our AI system.",
      })
    ...
    <AuthGuard allowedRoles={["student"]}>
      <ModernDashboardLayout>
        <div className="space-y-8">
                    ...
                    <div className="space-y-2">
                      <div
                        className={cn(
                          "flex items-center gap-2 text-sm p-2 rounded",
                          getDaysUntilDue(assignment.dueDate) <= 3 && "bg-red-50 text-red-700",
                          getDaysUntilDue(assignment.dueDate) <= 7 &&
                      ...
                      {selectedAssignment && (
                        <div
                          className={cn(
                            "flex items-center gap-2 px-4 py-2 rounded-lg text-base font-medium",
                            getDaysUntilDue(selectedAssignment.dueDate) <= 3 && "bg-red-50 text-red-700",
                            getDaysUntilDue(selectedAssignment.dueDate) <= 7 &&
          ...
          {/* Enhanced Submit Assignment Dialog */}
          <Dialog open={submitDialogOpen} onOpenChange={setSubmitDialogOpen}>
            <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
              <DialogHeader className="flex-shrink-0">
                ...
                <DialogDescription className="text-base">
                  Submit your work for <span className="font-semibold">{selectedAssignment?.title}</span>
                </DialogDescription>
              </DialogHeader>
                  ...
                  <div className="space-y-6">
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>Select your assignment file (PDF, DOCX, or TXT format)</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div
                        ...
                        >
                          ...
                          <p className="text-gray-500 mb-4">PDF, DOCX, or TXT files up to 10MB</p>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <Textarea
                          placeholder="Add any additional comments or notes for your instructor..."
                    ...
                    <Card>
                      <CardContent className="p-8">
                        <div className="text-center">
                          ...
                          <h3 className="text-xl font-medium text-gray-900 mb-2">Submitting your assignment...</h3>
                          <p className="text-gray-500 mb-4">
                            Our AI is processing your submission and checking for quality and originality.
                          </p>
